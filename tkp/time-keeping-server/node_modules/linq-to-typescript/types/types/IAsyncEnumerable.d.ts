import { IAsyncEqualityComparer, IAsyncParallel, IComparer, IEqualityComparer, IGrouping, InferType, IOrderedAsyncEnumerable, IParallelEnumerable, OfType, SelectorKeyType } from ".//index.js";
/**
...
export interface IAsyncEnumerable<TSource> extends IAsyncParallel<TSource> {
     ...
     */
    groupByWithSel<TKey extends SelectorKeyType, TElement>(keySelector: (x: TSource) => TKey, elementSelector: (x: TSource) => TElement, comparer?: IEqualityComparer<TKey>): IAsyncEnumerable<IGrouping<TKey, TElement>>;
    /**
     * Groups the elements of a sequence according to a key selector function.
     * The keys are compared by using a comparer and each group's elements are projected by using a specified function.
     * @param keySelector A function to extract the key for each element.
     ...
     */
    groupJoin<TInner, TKey, TResult>(inner: Iterable<TInner> | AsyncIterable<TInner>, outerKeySelector: (value: TSource) => TKey, innerKeySelector: (value: TInner) => TKey, resultSelector: (element: TSource, collection: TInner[]) => TResult, comparer?: IEqualityComparer<TKey>): IAsyncEnumerable<TResult>;
    /**
     * Correlates the elements of two sequences based on equality of keys and groups the results.
     ...
     * @returns An IAsyncEnumerable<T> that contains elements of type TResult that are obtained by performing a grouped join on two sequences.
     */
    groupJoinAsync<TInner, TKey, TResult>(inner: Iterable<TInner> | AsyncIterable<TInner>, outerKeySelector: (value: TSource) => TKey | Promise<TKey>, innerKeySelector: (value: TInner) => TKey | Promise<TKey>, resultSelector: (element: TSource, collection: TInner[]) => TResult | Promise<TResult>, comparer?: IEqualityComparer<TKey>): IAsyncEnumerable<TResult>;
    /**
     ...
     */
    zip<TSecond>(second: AsyncIterable<TSecond>): IAsyncEnumerable<[
        TSource,
        TSecond
    ]>;
    /**
